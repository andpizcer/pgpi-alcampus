...
def register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            mail_subject = "Activa tu cuenta en Alcampus para continuar"
            ...
            send_email = EmailMessage(
                ...
                body,
                to=[to_email],
                from_email="gestor.alcampus@gmail.com",
            )
            send_email.send()
...
def forgotPassword(request):
    if request.method == "POST":
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.success(
                request,
                "Un email fue enviado a tu bandeja de entrada para recuperar tu contraseña",
            )
            return redirect("login")
        else:
            messages.error(
                request, "La cuenta de usuario no existe o surgió un problema"
            )
    ...
    if request.method == "POST":
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                messages.error(
                    request, "Los datos no son válidos, ingresa una contraseña correcta"
                )
                return redirect("change_password")
        else:
