...
class IntegrationTests(TestCase):
    ...
    def test_add_to_cart_anonymous(self):
        response = self.client.post(
            reverse('add_cart', args=[self.product1.id]), follow=True
        )
    ...
    def test_place_order_anonymous_less_than_three_products(self):
        self.client.post(reverse('add_cart', args=[self.product1.id]))
        self.client.post(reverse('add_cart', args=[self.product2.id]))
    ...
    def test_place_order_anonymous_more_than_three_products(self):
        self.client.post(reverse('add_cart', args=[self.product1.id]))
        self.client.post(reverse('add_cart', args=[self.product2.id]))
        self.client.post(reverse('add_cart', args=[self.product3.id]))
        self.client.post(reverse('add_cart', args=[self.product3.id]))
        ...
        response = self.client.post(reverse('place_order'), data=order_data, follow=False)
